...
class PalEditorWidget(QWidget):
    ...
    def _create_pal(self):
        ...
        pal_list.setSpacing(0)
        pal_list.setUniformItemSizes(True)
        pal_list.setGridSize(QSize(80, 80))
    ...
    def _show_create_pal_dialog(self, slot_index, tab_name):
        try:
            ...
            pal_list.setDragDropMode(QAbstractItemView.NoDragDrop)
            pal_list.setMinimumHeight(350)
            pal_list.setStyleSheet('QListWidget::item { padding: 4px; border: 1px solid rgba(125,211,252,0.12); border-radius: 4px; margin: 2px; } QListWidget::item:hover { border: 1px solid rgba(125,211,252,0.3); background: rgba(125,211,252,0.05); } QListWidget::item:selected { background: rgba(59,142,208,0.3); border: 1px solid rgba(59,142,208,0.5); }')
            selected_pal = {'asset': None, 'name': None}
            def set_current_pal(item):
                if item:
                    selected_pal['asset'] = item.data(Qt.UserRole)
