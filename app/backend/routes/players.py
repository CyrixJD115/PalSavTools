...
async def set_player_level(uid: str, body: SetLevelRequest) -> dict:
    ...
    if player_service.set_player_level(level_dict, uid, body.level):
...
@router.get("/{uid}/tech-points", response_model=PlayerTechPointsResponse)
async def get_player_tech_points(uid: str) -> PlayerTechPointsResponse:
    _, players_dir, _, _ = _require()
    result = player_service.get_player_tech_points(players_dir, uid)
    if result is None:
        raise HTTPException(404, f"Player .sav not found for: {uid}")
    return PlayerTechPointsResponse(**result)
...
@router.get("/{uid}/stats", response_model=PlayerStatsResponse)
async def get_player_stats(uid: str) -> PlayerStatsResponse:
    level_dict, _, _, _ = _require()
    result = player_service.get_player_stats(level_dict, uid)
    if result is None:
...
async def reset_player_timestamp(uid: str) -> dict:
    level_dict, _, _, _ = _require()
    if not player_service.reset_player_timestamp(level_dict, uid):
        raise HTTPException(404, f"Could not reset timestamp for player: {uid}")
    return {"status": "ok"}
...
async def max_player_abilities(body: MaxAbilitiesRequest) -> dict:
    ...
    result = player_service.max_all_abilities(level_dict, players_dir, body.uids)
    resp: dict = {"status": "ok", "processed": result["processed"]}
    if result["failed"]:
