<script lang="ts">
  ...
  const rarityLabels: Record<number, string> = {
    0: 'web.wiki.rarity_common',
    1: 'web.wiki.rarity_uncommon',
    2: 'web.wiki.rarity_rare',
    3: 'web.wiki.rarity_epic',
    4: 'web.wiki.rarity_legendary',
  };
</script>
...
<div class="flex flex-col h-full gap-3">
  <div class="flex items-center gap-3 flex-wrap">
    <div class="relative flex-1 max-w-xs">
      ...
      <input type="text" bind:value={search} placeholder={$t('web.wiki.search_items')} class="input pl-8 text-xs" />
    </div>
    <div class="flex gap-1 flex-wrap">
      {#each types as type_}
        <button class="chip text-[10px] cursor-pointer hover:bg-bg-hover hover:border-line/80 transition-fast {typeFilter === type_ ? 'chip-blue' : ''}" onclick={() => (typeFilter = type_)}>{type_ === 'All' ? $t('web.wiki.all') : type_}</button>
      {/each}
    </div>
    <span class="text-[11px] text-ink-dim ml-auto tabular-nums">{$t('web.wiki.items_count', { count: filtered.length })}</span>
  </div>
      ...
      <table class="w-full text-xs">
        <thead class="bg-bg-deep sticky top-0 z-10">
          <tr class="border-b-2 border-line/40">
            ...
            <th class="text-left px-3 py-2 text-[10px] font-semibold text-ink-dim uppercase tracking-wider">{$t('web.wiki.col_type')}</th>
            <th class="text-left px-3 py-2 text-[10px] font-semibold text-ink-dim uppercase tracking-wider">{$t('web.wiki.col_rarity')}</th>
            <th class="text-right px-3 py-2 text-[10px] font-semibold text-ink-dim uppercase tracking-wider cursor-pointer select-none hover:text-ink-primary transition-fast" onclick={() => toggleSort('weight')}>
              {$t('web.wiki.col_weight')} {sortKey === 'weight' ? (sortAsc ? '▲' : '▼') : ''}
            </th>
            <th class="text-right px-3 py-2 text-[10px] font-semibold text-ink-dim uppercase tracking-wider cursor-pointer select-none hover:text-ink-primary transition-fast" onclick={() => toggleSort('price')}>
              {$t('web.wiki.col_price')} {sortKey === 'price' ? (sortAsc ? '▲' : '▼') : ''}
            </th>
            <th class="text-right px-3 py-2 text-[10px] font-semibold text-ink-dim uppercase tracking-wider">{$t('web.wiki.col_max_stack')}</th>
          </tr>
        </thead>
        <tbody>
          {#each filtered as item}
            <tr class="border-b border-line/20 hover:bg-bg-hover transition-fast">
              ...
              <td class="px-3 py-2"><span class="font-semibold {rarityColors[item.rarity ?? 0] ?? 'text-ink-muted'}">{$t(rarityLabels[item.rarity ?? 0] ?? 'web.wiki.rarity_common')}</span></td>
