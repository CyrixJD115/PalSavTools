<script lang="ts">
  ...
  async function doRemoveMember(uid: string) {
    if (!confirm(`Remove member ${uid.slice(0, 13)}… from this guild?`)) return;
    await doAction('remove-member', () => api.removeGuildMember(guild.id, uid));
  }

  async function doDelete() {
    if (!confirm(`Delete guild "${detail?.name}" and all its bases? This cannot be undone.`)) return;
  ...
  >
    ...
    {:else if detail}
      ...
      <div class="grid grid-cols-3 gap-4 p-4 border-b border-line/20 text-sm">
        ...
        <div><span class="text-ink-muted">Bases:</span> <span class="tabular-nums">{detail.base_count}</span></div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="text-left text-xs uppercase tracking-wider text-ink-muted border-b border-line/30">
              ...
              <th class="py-1.5 pr-3 font-medium font-mono">UID</th>
      ...
      <div class="p-4 space-y-3">
        ...
        {#if editingName}
          <div class="flex gap-2 items-center">
            <input class="input flex-1 text-sm" bind:value={nameValue} placeholder="New guild name" />
        ...
        {#if editingLeader}
          <div class="flex gap-2 items-center">
            <input class="input flex-1 text-sm" bind:value={leaderUid} placeholder="Player UID" />
            <Button variant="primary" onclick={doSetLeader} disabled={actionLoading !== null}>Promote</Button>
            <Button variant="ghost" onclick={() => editingLeader = false}>Cancel</Button>
          </div>
        {:else}
          <Button variant="secondary" onclick={startLeader} disabled={actionLoading !== null}>
            <Icon icon="lucide:crown" width={14} class="mr-1" /> Set Leader
          </Button>
        {/if}
        ...
        <div class="border-t border-line/20 pt-3">
          {#if confirmDelete}
            <div class="flex items-center gap-2 text-sm">
              <span class="text-status-error">Delete this guild and all bases?</span>
              <Button variant="danger" onclick={doDelete} disabled={actionLoading !== null}>Confirm</Button>
          ...
          {:else}
            <Button variant="danger" onclick={() => confirmDelete = true} disabled={actionLoading !== null}>
              <Icon icon="lucide:trash-2" width={14} class="mr-1" /> Delete Guild
            </Button>
          {/if}
        </div>
      </div>
