...
def decode_bytes(parent_reader: FArchiveReader, group_bytes: Sequence[int], group_type: str) -> dict[str, Any]:
    ...
    if group_type == 'EPalGroupType::Guild':
        ...
        if len(remaining) >= 20 and remaining[:4] == b'\x00\x00\x00\x00':
            parsed = parse_guild(True)
        else:
            parsed = parse_guild(False)
        if not parsed['players']:
            admin_end = 20 if len(remaining) >= 20 and remaining[:4] == b'\x00\x00\x00\x00' else 16
            fallback_raw = remaining[admin_end:]
            fallback_players = _extract_players_from_unknown(fallback_raw) or _extract_players_from_old_unknown(fallback_raw) or []
            if fallback_players:
                parsed['players'] = fallback_players
        guild |= parsed
        group_data |= guild
