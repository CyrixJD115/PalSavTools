<script lang="ts">
  import { health, saveLoaded, isHealthy, t } from '$stores/index';
...
<header class="flex items-center gap-3 h-14 px-4 border-b border-line/50 bg-header-gradient shrink-0">
  <div class="flex items-center gap-2 min-w-0">
    ...
    <span class="version-chip version-green" title={$t('web.header.palworld_version')}>
  ...
  <div class="flex items-center gap-1">
    <button class="hdr-btn hdr-info" title={$t('web.header.about')} onclick={() => (aboutOpen = true)}>
      <Icon icon="lucide:info" width={16} />
    </button>
    <button class="hdr-btn hdr-warn" title={$t('web.header.warnings')} onclick={() => (warnOpen = true)}>
      <Icon icon="lucide:triangle-alert" width={16} />
    </button>
    <button class="hdr-btn hdr-toolbox" title={$t('web.header.usage_guide')}>
  ...
  <div class="flex items-center gap-3 min-w-0">
    {#if $saveLoaded}
      <span class="badge bg-status-success/15 border-status-success/40 text-status-success">
        ...
        {$t('web.header.save_loaded')}
      </span>
    {:else}
      <span class="badge bg-bg-elevated border-line text-ink-muted">{$t('web.header.no_save_loaded')}</span>
    {/if}
  ...
  <div class="flex items-center gap-2">
    <button class="hdr-btn hdr-settings" title={$t('web.header.settings')} onclick={() => goto('/settings')}>
      <Icon icon="lucide:settings" width={16} />
    </button>
    <a href="https://discord.gg/sYcZwcT4cT" target="_blank" rel="noreferrer"
       class="discord-link" title={$t('web.header.discord')}>
    ...
    <div class="flex items-center gap-1.5 text-xs">
      ...
      <span class="{$isHealthy ? 'text-status-success' : 'status-error'}">
        {$isHealthy ? $t('web.header.online') : $t('web.header.offline')}
      </span>
    </div>
  </div>
</header>
