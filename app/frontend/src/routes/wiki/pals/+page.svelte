...
<div class="flex h-full gap-4">
  <div class="w-64 shrink-0 flex flex-col bg-bg-deep/25 rounded-4 p-2">
    ...
    <div class="relative mb-3 px-1">
      ...
      <input type="text" bind:value={search} placeholder={$t('web.wiki.search_pals')} class="input pl-8 text-xs" />
    </div>
    <div class="flex flex-wrap gap-1 mb-3 px-1">
      {#each elements as el}
        ...
        >{el === 'All' ? $t('web.wiki.all') : el}</button>
      {/each}
    </div>
  ...
  <div class="flex-1 overflow-y-auto">
    ...
    {:else if selected}
      <div class="card space-y-4">
        ...
        {#if Object.keys(selected.stats ?? {}).length}
          <div>
            <h3 class="text-[11px] font-semibold text-ink-dim uppercase tracking-wider mb-2">{$t('web.wiki.base_stats')}</h3>
        ...
        {#if Object.keys(selected.work_suitabilities ?? {}).length}
          <div>
            <h3 class="text-[11px] font-semibold text-ink-dim uppercase tracking-wider mb-2">{$t('web.wiki.work_suitability')}</h3>
        ...
        {#if selected.partner_skill}
          <div>
            <h3 class="text-[11px] font-semibold text-ink-dim uppercase tracking-wider mb-2">{$t('web.wiki.partner_skill')}</h3>
    ...
    {:else}
      <div class="flex flex-col items-center justify-center h-full text-ink-dim gap-2">
        ...
        <p class="text-xs">{$t('web.wiki.select_pal')}</p>
      </div>
    {/if}
  </div>
</div>
