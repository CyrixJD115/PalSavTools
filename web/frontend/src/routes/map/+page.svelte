<script lang="ts">
  ...
  async function loadData() {
    ...
    try {
      ...
      mapError.set(e instanceof Error ? e.message : 'Failed to load map data');
    } finally {
  ...
  function handleMarkerRightClick(m: RuntimeMarker, sx: number, sy: number) {
    ...
    if (m.kind === 'base') {
      contextMenuItems = [
        { label: 'Delete Base', icon: 'lucide:trash-2', danger: true, action: () => deleteBase(m) },
        { label: 'Export Base', icon: 'lucide:download', action: () => exportBase(m) },
        { label: 'Adjust Radius', icon: 'lucide:circle-dashed', action: () => adjustRadius(m) },
      ];
    } else {
      contextMenuItems = [
        { label: 'Delete Player', icon: 'lucide:trash-2', danger: true, action: () => deletePlayer(m) },
        { label: 'Rename Player', icon: 'lucide:pencil', action: () => renamePlayer(m) },
        { label: 'Unlock Viewing Cage', icon: 'lucide:unlock', action: () => unlockCage(m) },
        { label: 'Unlock Technologies', icon: 'lucide:sparkles', action: () => unlockTech(m) },
      ];
    }
    contextMenuX = sx;
    contextMenuY = sy;
  ...
  function handleEmptyRightClick(sx: number, sy: number, imgX: number, imgY: number) {
    ...
    contextMenuItems = [
      { label: 'Draw Zones', icon: 'lucide:hexagon', action: startZoneDrawing },
      { label: 'Clear All Zones', icon: 'lucide:eraser', danger: true, action: () => {
        clear_all_zones();
        toast.success('All zones cleared');
      }},
      { label: '', action: () => {}, separator: true },
      { label: 'Export Zones', icon: 'lucide:download', action: exportZones },
      { label: 'Import Zones', icon: 'lucide:upload', action: importZones },
    ];
    contextMenuX = sx;
    contextMenuY = sy;
    contextMenuOpen = true;
  ...
  function handleZoneRightClick(z: Zone, sx: number, sy: number) {
    contextMenuItems = [
      { label: `Delete "${z.name}"`, icon: 'lucide:trash-2', danger: true, action: () => {
        remove_zone(z.id);
        toast.success(`Zone "${z.name}" deleted`);
      }},
      { label: 'Rename Zone', icon: 'lucide:pencil', action: () => {
        const name = window.prompt('Enter new zone name:', z.name);
        if (name && name.trim()) {
  ...
  function startZoneDrawing() {
    ...
    showZones.set(true);
    zonePromptText = 'Zone Drawing: Double-click Point A, then double-click Point B to create. Right-click to finish.';
  }
    ...
    input.onchange = () => {
      ...
      reader.onload = () => {
        if (import_zones_json(reader.result as string)) {
          toast.success('Zones imported successfully');
        } else {
          toast.error('Failed to import zones: invalid file');
        }
      };
  ...
  async function deleteBase(m: RuntimeMarker) {
    ...
    toast.info(`Delete base feature requires backend endpoint (coming soon)`);
  }

  async function exportBase(m: RuntimeMarker) {
    if (m.kind !== 'base') return;
    toast.info(`Export base feature requires backend endpoint (coming soon)`);
  }

  async function adjustRadius(m: RuntimeMarker) {
    if (m.kind !== 'base') return;
    toast.info(`Adjust radius feature requires backend endpoint (coming soon)`);
  }

  async function deletePlayer(m: RuntimeMarker) {
    if (m.kind !== 'player') return;
    toast.info(`Delete player feature requires backend endpoint (coming soon)`);
  }

  async function renamePlayer(m: RuntimeMarker) {
    if (m.kind !== 'player') return;
    const name = window.prompt('Enter new player name:', m.data.name);
    if (name && name.trim()) {
      toast.info(`Rename player feature requires backend endpoint (coming soon)`);
    }
  }

  async function unlockCage(m: RuntimeMarker) {
    toast.info(`Unlock viewing cage requires backend endpoint (coming soon)`);
  }

  async function unlockTech(m: RuntimeMarker) {
    toast.info(`Unlock technologies requires backend endpoint (coming soon)`);
  }
        ...
        <div class="flex items-center gap-1 ml-2">
          ...
          <button
            ...
            onclick={() => {
              ...
              zonePromptText = 'Polygon: Double-click to start, single-click to add points, double-click to close.';
            }}
          >
            Polygon
          </button>
    ...
    {#if $mapLoading}
      <div class="absolute inset-0 z-40 flex items-center justify-center bg-bg-deep/80 backdrop-blur">
        <div class="text-center">
          ...
          <div class="text-xs text-ink-muted">Loading map data...</div>
        </div>
      </div>
    {/if}
  </div>
