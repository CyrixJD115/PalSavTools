...
<div class="flex flex-col h-full gap-3">
  <div class="flex items-center gap-3 flex-wrap">
    <div class="relative flex-1 max-w-xs">
      ...
      <input type="text" bind:value={search} placeholder={$t('web.wiki.search_skills')} class="input pl-8 text-xs" />
    </div>
    <div class="flex gap-1 flex-wrap items-center">
      <span class="text-[10px] text-ink-dim font-semibold uppercase tracking-wider mr-1">{$t('web.wiki.filter_element')}</span>
    ...
    <span class="text-[11px] text-ink-dim ml-auto tabular-nums">{$t('web.wiki.skills_count', { count: filtered.length })}</span>
  </div>
      ...
      <table class="w-full text-xs">
        <thead class="bg-bg-deep sticky top-0 z-10">
          <tr class="border-b-2 border-line/40">
            ...
            <th class="text-left px-3 py-2 text-[10px] font-semibold text-ink-dim uppercase tracking-wider">{$t('web.wiki.col_element')}</th>
            <th class="text-right px-3 py-2 text-[10px] font-semibold text-ink-dim uppercase tracking-wider cursor-pointer select-none hover:text-ink-primary transition-fast" onclick={() => toggleSort('power')}>
              {$t('web.wiki.col_power')} {sortKey === 'power' ? (sortAsc ? '▲' : '▼') : ''}
            </th>
            <th class="text-right px-3 py-2 text-[10px] font-semibold text-ink-dim uppercase tracking-wider cursor-pointer select-none hover:text-ink-primary transition-fast" onclick={() => toggleSort('cooldown')}>
              {$t('web.wiki.col_cooldown')} {sortKey === 'cooldown' ? (sortAsc ? '▲' : '▼') : ''}
            </th>
            <th class="text-left px-3 py-2 text-[10px] font-semibold text-ink-dim uppercase tracking-wider">{$t('web.wiki.col_category')}</th>
