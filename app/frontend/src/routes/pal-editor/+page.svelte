...
<div class="p-6 max-w-7xl mx-auto space-y-4 animate-fade-in">
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-sm text-ink-muted mt-1">{$t('web.pal_editor.count', { count: total })}</p>
    </div>
    {#if total > 0}
      <div class="relative">
        <Icon icon="lucide:search" width={14} class="absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted" />
        <input
          type="text" placeholder={$t('web.pal_editor.search_placeholder')}
          bind:value={search}
  ...
  {:else if loading}
    <div class="text-center text-ink-muted py-12">{$t('web.pal_editor.loading')}</div>
  {:else if filtered.length === 0}
    <EmptyState icon="lucide:sparkles" title={$t('web.pal_editor.none_title')}>
      {$t('web.pal_editor.none_body')}
    </EmptyState>
  {:else}
