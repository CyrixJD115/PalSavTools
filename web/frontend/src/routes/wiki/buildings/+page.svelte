    ...
    <div class="flex-1 overflow-y-auto space-y-0.5">
      {#each filtered as b}
        ...
        >
          ...
          {#if b.rank}
            <span class="chip text-[10px] ml-auto shrink-0">Tier {b.rank}</span>
          {/if}
        </button>
      {/each}
    </div>
  ...
  <div class="flex-1 overflow-y-auto">
      ...
      <div class="card space-y-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              {#if selected.rank}
                <span class="chip chip-amber text-[10px]">Tier {selected.rank}</span>
              {/if}
            </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <div class="bg-bg-deep border-2 border-line/40 rounded-4 px-3 py-2">
            <p class="text-[10px] text-ink-dim uppercase">HP</p>
            <p class="text-sm font-bold text-ink-emphasis">{selected.hp ?? 0}</p>
          </div>
          <div class="bg-bg-deep border-2 border-line/40 rounded-4 px-3 py-2">
            <p class="text-[10px] text-ink-dim uppercase">Defense</p>
            <p class="text-sm font-bold text-ink-emphasis">{selected.defense ?? 0}</p>
          </div>
          <div class="bg-bg-deep border-2 border-line/40 rounded-4 px-3 py-2">
            <p class="text-[10px] text-ink-dim uppercase">Work Amount</p>
            <p class="text-sm font-bold text-ink-emphasis">{selected.required_work_amount ?? 0}</p>
          </div>
          <div class="bg-bg-deep border-2 border-line/40 rounded-4 px-3 py-2">
            <p class="text-[10px] text-ink-dim uppercase">Deterioration</p>
        ...
        {#if selected.materials?.length}
          <div>
            <h3 class="text-[11px] font-semibold text-ink-dim uppercase tracking-wider mb-2">Materials</h3>
    ...
    {:else}
      <div class="flex flex-col items-center justify-center h-full text-ink-dim gap-2">
        ...
        <p class="text-xs">Select a building to view details</p>
      </div>
    {/if}
  </div>
</div>
