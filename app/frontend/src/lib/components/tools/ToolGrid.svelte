<script lang="ts">
  ...
  let categories = $derived<{ labelKey: string; key: string; tools: ToolInfo[] }[]>([
    {
      labelKey: 'web.category.converting',
      key: 'converting',
      tools: tools.filter((t) => t.category === 'converting'),
    },
    {
      labelKey: 'web.category.management',
      key: 'management',
      tools: tools.filter((t) => t.category === 'management'),
    },
    {
      labelKey: 'web.category.utility',
      key: 'utility',
