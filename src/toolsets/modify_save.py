...
logger = logging.getLogger("pst.modify_save")
...
def modify_save(tool=None):
    """Launch an external Palworld save editor tool.

    Parameters
    ----------
    tool : str or None
        - ``"savepal"`` or ``"psp"``  -> launch Palworld Save Pal
        - ``"paleditor"`` or ``"ppe"`` -> launch Palworld Pal Editor
        - ``None`` (default) -> print available options

    Returns
    -------
    bool
        True if the tool was launched (or download initiated), False otherwise.
    """
    if tool is None:
        print("Available external tools:")
        print("  savepal / psp   - Palworld Save Pal (Windows only)")
        print("  paleditor / ppe - Palworld Pal Editor (Windows only)")
        print()
        print("Pass the tool name as an argument, e.g.:")
        print("  modify_save('savepal')")
        return True

    tool = tool.lower().replace("-", "").replace(" ", "")

    if tool in ("savepal", "psp"):
        _launch_save_pal()
        return True
    elif tool in ("paleditor", "ppe"):
        _launch_pal_editor()
        return True
    else:
        logger.warning("Unknown tool '%s'. Use 'savepal' or 'paleditor'.", tool)
        return False


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Launch external Palworld save editors (CLI version)"
    )
    parser.add_argument(
        "tool",
        nargs="?",
        default=None,
        help="Which tool to launch: 'savepal' (or 'psp') / 'paleditor' (or 'ppe')"
    )
    args = parser.parse_args()
    modify_save(args.tool)