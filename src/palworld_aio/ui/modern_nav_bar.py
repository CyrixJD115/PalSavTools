...
from PySide6.QtCore import Signal, Qt, QPropertyAnimation, QEasingCurve, QRect, QPoint, QSize
...
class NavButton(QPushButton):
    ...
    def __init__(self, button_id, icon_code, label, parent=None):
        ...
        self.setFont(QFont('Hack Nerd Font', 13))
        self.setFixedHeight(38)
        self.setText(f' {icon_code}  {label}')
    ...
    def text_width(self):
        fm = self.fontMetrics()
        return fm.horizontalAdvance(self.text()) + 32
...
class ModernNavBar(QWidget):
    ...
    def set_active(self, button_id):
        ...
        self._animate_underline_to(button_id)
    def _animate_underline_to(self, button_id):
        btn = self._buttons[button_id]
        r = btn.geometry()
        margin = 10
        y = self.height() - 4
        h = 3
        target = QRect(r.x() + margin, y, r.width() - 2 * margin, h)
