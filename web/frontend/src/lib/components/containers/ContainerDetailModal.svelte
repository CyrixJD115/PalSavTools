<script lang="ts">
  ...
  async function doClear() {
    if (!confirm(`Clear all ${detail?.item_count ?? 0} items from this container?`)) return;
  ...
  >
    ...
    {:else if detail}
      ...
      <div class="grid grid-cols-2 gap-4 p-4 border-b border-line/20 text-sm">
        <div><span class="text-ink-muted">Items:</span> <span class="tabular-nums text-ink-primary font-medium">{detail.item_count}</span></div>
        <div><span class="text-ink-muted">Slots:</span> <span class="tabular-nums">{detail.slot_count}</span></div>
        <div><span class="text-ink-muted">Guild:</span> <span>{container.guild_name ?? container.guild_id?.slice(0, 13) ?? '—'}</span></div>
        <div class="col-span-2">
          <span class="text-ink-muted">Container ID:</span>
      ...
      <div class="p-4 space-y-3">
        ...
        <Button variant="danger" onclick={doClear} disabled={actionLoading !== null || detail.item_count === 0}>
          <Icon icon="lucide:eraser" width={14} class="mr-1" /> Clear All Items
        </Button>
        ...
        {#if expandEdit}
          <div class="flex gap-2 items-center">
            ...
            <Button variant="primary" onclick={doExpand} disabled={actionLoading !== null}>Set</Button>
            <Button variant="ghost" onclick={() => expandEdit = false}>Cancel</Button>
          </div>
        {:else}
          <Button variant="secondary" onclick={() => { if (detail) { expandValue = detail.slot_count; } expandEdit = true; }} disabled={actionLoading !== null}>
            <Icon icon="lucide:maximize" width={14} class="mr-1" /> Expand Capacity
          </Button>
        {/if}
      </div>
      ...
      {#if detail.items.length > 0}
        <div class="border-t border-line/20 p-4 space-y-2">
          <p class="text-xs uppercase tracking-wider text-ink-muted font-medium">Items ({detail.items.length})</p>
