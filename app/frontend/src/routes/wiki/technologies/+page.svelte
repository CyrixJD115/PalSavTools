...
<div class="flex h-full gap-4">
  <div class="w-64 shrink-0 flex flex-col bg-bg-deep/25 rounded-4 p-2">
    ...
    <div class="relative mb-3 px-1">
      ...
      <input type="text" bind:value={search} placeholder={$t('web.wiki.search_technologies')} class="input pl-8 text-xs" />
    </div>
    <div class="flex gap-1 mb-3 px-1">
      <button class="chip text-[10px] cursor-pointer hover:bg-bg-hover hover:border-line/80 transition-fast {bossFilter === 'All' ? 'chip-blue' : ''}" onclick={() => (bossFilter = 'All')}>{$t('web.wiki.all')}</button>
      <button class="chip text-[10px] cursor-pointer hover:bg-bg-hover hover:border-line/80 transition-fast {bossFilter === 'normal' ? 'chip-blue' : ''}" onclick={() => (bossFilter = 'normal')}>{$t('web.wiki.normal')}</button>
      <button class="chip text-[10px] cursor-pointer hover:bg-bg-hover hover:border-line/80 transition-fast {bossFilter === 'boss' ? 'chip-blue' : ''}" onclick={() => (bossFilter = 'boss')}>{$t('web.wiki.boss')}</button>
    </div>
    <div class="flex-1 overflow-y-auto space-y-0.5">
      {#each filtered as tech}
        ...
        >
          ...
          {#if tech.tier}
            <span class="chip text-[10px] ml-auto shrink-0">{$t('web.wiki.tier_n', { rank: tech.tier })}</span>
          {/if}
        </button>
      {/each}
    </div>
  ...
  <div class="flex-1 overflow-y-auto">
      ...
      <div class="card space-y-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <div class="flex items-center gap-1 mt-1">
              <span class="chip chip-amber text-[10px]">{$t('web.wiki.tier_n', { rank: selected.tier })}</span>
              <span class="chip text-[10px]">{selected.type}</span>
              <span class="chip {selected.is_boss_tech ? 'chip-purple' : 'chip-green'} text-[10px]">{selected.is_boss_tech ? $t('web.wiki.boss') : $t('web.wiki.normal')}</span>
            </div>
        ...
        <div class="grid grid-cols-3 gap-2">
          <div class="bg-bg-deep border-2 border-line/40 rounded-4 px-3 py-2">
            <p class="text-[10px] text-ink-dim uppercase">{$t('web.common.level')}</p>
            <p class="text-sm font-bold text-ink-emphasis">{selected.level_cap}</p>
          </div>
          <div class="bg-bg-deep border-2 border-line/40 rounded-4 px-3 py-2">
            <p class="text-[10px] text-ink-dim uppercase">{$t('web.wiki.col_tech_points')}</p>
            <p class="text-sm font-bold text-ink-emphasis">{selected.cost}</p>
          </div>
          <div class="bg-bg-deep border-2 border-line/40 rounded-4 px-3 py-2">
            <p class="text-[10px] text-ink-dim uppercase">{$t('web.wiki.col_prerequisites')}</p>
        ...
        {#if selected.require_technology?.length}
          <div>
            <h3 class="text-[11px] font-semibold text-ink-dim uppercase tracking-wider mb-2">{$t('web.wiki.section_prerequisites')}</h3>
        ...
        {#if selected.unlock_build_objects?.length}
          <div>
            <h3 class="text-[11px] font-semibold text-ink-dim uppercase tracking-wider mb-2">{$t('web.wiki.section_unlocks_buildings')}</h3>
            <div class="flex flex-wrap gap-1">
        ...
        {#if selected.unlock_item_recipes?.length}
          <div>
            <h3 class="text-[11px] font-semibold text-ink-dim uppercase tracking-wider mb-2">{$t('web.wiki.section_unlocks_items')}</h3>
            <div class="flex flex-wrap gap-1">
    ...
    {:else}
      <div class="flex flex-col items-center justify-center h-full text-ink-dim gap-2">
        ...
        <p class="text-xs">{$t('web.wiki.select_technology')}</p>
      </div>
    {/if}
  </div>
</div>
