      ...
      <div class="flex items-center gap-2">
        ...
        <div class="flex items-center gap-1.5 shrink-0">
          {#if tool.category === 'converting'}
            <span class="chip chip-blue text-[10px]">{$t('web.category.converting')}</span>
          {:else if tool.category === 'management'}
            <span class="chip chip-amber text-[10px]">{$t('web.category.management')}</span>
          {:else}
            <span class="chip chip-green text-[10px]">{$t('web.category.utility')}</span>
          {/if}
          {#if tool.windows_only}
            <span class="chip text-[10px] bg-purple-500/10 text-purple-400">{$t('web.category.windows')}</span>
          {/if}
        </div>
      </div>
