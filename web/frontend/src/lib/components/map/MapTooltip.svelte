  ...
  >
    {#if marker.kind === 'base'}
      ...
      <div class="px-3 py-2 space-y-0.5 text-[11px]">
        ...
        <div class="text-ink-muted">Base ID: <span class="text-ink-secondary font-mono text-[10px]">{b.id.slice(0, 12)}...</span></div>
        <div class="text-ink-muted">Location: <span class="text-ink-secondary font-mono">X:{Math.round(marker.world_x)},Y:{Math.round(marker.world_y)}</span></div>
      </div>
    {:else}
      {@const p = marker.data}
      <div class="px-3 py-2 space-y-0.5 text-[11px]">
        <div class="font-bold text-sm text-emerald-400 truncate">{p.name}</div>
        <div class="text-ink-muted">UID: <span class="text-ink-secondary font-mono text-[10px]">{p.uid.slice(0, 12)}...</span></div>
        {#if p.level > 0}
