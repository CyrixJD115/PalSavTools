...
def main():
    ...
    # Union of all FT GUIDs across all scanned player saves
    ft_guids = set.union(*per_player_ft) if per_player_ft else set()

    # Merge with existing reference to preserve known-good points
    ...
    result = {
        'FastTravelPointUnlockFlag_guids': sorted(ft_guids),
        'FindAreaFlagMap_keys': sorted(area_keys),
        'UnlockedWorldMapFlags': world_flags or {'MainMap': True, 'Tree': True},
        'AreaBarrierUnlockFlags_guids': sorted(area_barrier),
    }
