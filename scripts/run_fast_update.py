...
from update_game_data import update_pal_data, update_item_data, update_npc_data, update_structure_data, update_passive_data, update_skill_data, update_technology_data, update_pal_passive_data, update_pal_exp_table, update_items_psp, ensure_dir, RESOURCES_DIR, ICONS_DIR
...
def fast_find_and_copy(search_name, target_subdir, export_subdirs):
    ...
    no_zero = _re.sub('_0+(\\d)', '_\\1', name_lower)
    ...
    if not name_lower.startswith('t_itemicon_'):
        tier_stripped = _re.sub('_\\d+$', '', name_lower)
