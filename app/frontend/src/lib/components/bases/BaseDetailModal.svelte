<script lang="ts">
  ...
  async function load() {
    ...
    try { detail = await api.baseDetail(base.id); }
      ...
      error = msg.startsWith('API 404')
        ? $t('web.protection.base_not_found')
        : msg;
  ...
  async function handleDelete() {
    if (!confirm($t('web.bases.delete_confirm', { name: detail?.guild_name ?? 'Unknown' }))) return;
  ...
  >
    ...
    {:else if detail}
      ...
      <div class="grid grid-cols-2 gap-4 p-4 border-b border-line/20 text-sm">
        <div><span class="text-ink-muted">{$t('web.bases.detail_guild')}</span> <Badge tone="accent">{detail.guild_name ?? '—'}</Badge></div>
        <div><span class="text-ink-muted">{$t('web.bases.detail_guild_level')}</span> <span class="tabular-nums text-ink-primary font-medium">{detail.guild_level}</span></div>
        <div><span class="text-ink-muted">{$t('web.bases.detail_leader')}</span> <span>{detail.leader_name ?? '—'}</span></div>
        <div><span class="text-ink-muted">{$t('web.bases.detail_members')}</span> <span class="tabular-nums">{detail.member_count}</span></div>
        <div><span class="text-ink-muted">{$t('web.bases.detail_base')}</span> <span class="tabular-nums">{detail.base_position}/{detail.total_bases}</span></div>
        <div><span class="text-ink-muted">{$t('web.bases.detail_area_range')}</span> <span class="tabular-nums font-mono text-xs">{(detail.area_range / 100).toFixed(0)}m</span></div>
        <div class="col-span-2"><span class="text-ink-muted">{$t('web.bases.detail_location')}</span> <span class="font-mono text-xs text-ink-secondary">{fmtCoord(detail.location)}</span></div>
        <div class="col-span-2">
          <span class="text-ink-muted">{$t('web.bases.detail_base_id')}</span>
      ...
      <div class="p-4 space-y-3">
        ...
        {#if editingGuildName}
          <div class="flex gap-2 items-center">
            <input class="input flex-1 text-sm" bind:value={guildNameValue} placeholder={$t('web.bases.rename_guild')} />
            <Button variant="primary" onclick={doRenameGuild} disabled={actionLoading !== null}>{$t('web.common.save')}</Button>
            <Button variant="ghost" onclick={() => editingGuildName = false}>{$t('web.common.cancel')}</Button>
          </div>
        {:else}
          <Button variant="secondary" onclick={startGuildName} disabled={actionLoading !== null}>
            <Icon icon="lucide:pencil" width={14} class="mr-1" /> {$t('web.bases.rename_guild')}
          </Button>
        {/if}
        ...
        {#if editingGuildLevel}
          <div class="flex gap-2 items-center">
            ...
            <Button variant="primary" onclick={doSetGuildLevel} disabled={actionLoading !== null}>{$t('web.common.set')}</Button>
            <Button variant="ghost" onclick={() => editingGuildLevel = false}>{$t('web.common.cancel')}</Button>
          </div>
        {:else}
          <Button variant="secondary" onclick={startGuildLevel} disabled={actionLoading !== null}>
            <Icon icon="lucide:trending-up" width={14} class="mr-1" /> {$t('web.bases.set_guild_level')}
          </Button>
        {/if}
        ...
        {#if editingRadius}
          <div class="flex gap-2 items-center flex-wrap">
            ...
            <Button variant="primary" onclick={doSetRadius} disabled={actionLoading !== null}>{$t('web.common.set')}</Button>
            <Button variant="ghost" onclick={() => editingRadius = false}>{$t('web.common.cancel')}</Button>
          </div>
        {:else}
          <Button variant="secondary" onclick={startRadius} disabled={actionLoading !== null}>
            <Icon icon="lucide:maximize-2" width={14} class="mr-1" /> {$t('web.bases.adjust_area_range')}
          </Button>
        {/if}
        ...
        <div class="border-t border-line/20 pt-3 space-y-2">
          <label class="flex items-center gap-2 text-xs text-ink-muted cursor-pointer">
            ...
            {$t('web.bases.also_delete_workers')}
          </label>
          <Button variant="danger" onclick={handleDelete} disabled={actionLoading !== null}>
            <Icon icon="lucide:trash-2" width={14} class="mr-1" /> {$t('web.bases.delete_base')}
          </Button>
        </div>
      </div>
    {/if}
