"""
save_manager — headless save load / save / scan management.

Removed all Qt dependencies (QObject, Signal, QFileDialog).
The SaveManager class is now a plain object; its public methods accept
path parameters directly instead of opening file dialogs.
"""
...
logger = logging.getLogger("pst.save_manager")


class SaveManager:
    """Headless save manager — no Qt, no signals, no GUI file dialogs."""
    ...
    def load_save(self, path=None):
        """
        Load a save file from *path*.

        If *path* is None, logs an error and returns False.
        ...
        if not constants.loaded_level_json:
            logger.error("Failed to parse save file")
            return False
        ...
        player_pals_count = {}
        illegal_pals_by_owner, owner_nicknames = self._count_pals_found(
        ...
        self._process_scan_log(
            data_source, playerdir, log_folder, guild_name_map, base_path,
            illegal_pals_by_owner, owner_nicknames,
        )

        from palworld_aio.validation.engine import snapshot
        snapshot()

        logger.info("Save loaded successfully from %s", p)
        return True

    def reload_current_save(self):
        """Reload the currently-loaded save from disk."""
    ...
    def save_changes(self):
        ...
        if not constants.current_save_path or not constants.loaded_level_json:
            logger.warning("No save loaded, nothing to save")
            return
        ...
        duration = t1 - t0
        logger.info("Changes saved in %.2f seconds", duration)
        return duration
    ...
    def _build_player_levels(self):
        """Populate constants.player_levels and player_character_cache."""
        char_map = (
    ...
    def _count_pals_found(
        self, data, player_pals_count, log_folder, current_save_path,
        guild_name_map, illegal_pals_by_owner=None,
    ):
        """Scan and count pals found in the save data."""
        base_dir = "."
        if illegal_pals_by_owner is None:
            illegal_pals_by_owner = defaultdict(lambda: defaultdict(list))
        else:
            illegal_pals_by_owner = defaultdict(
                lambda: defaultdict(list), illegal_pals_by_owner
            )
        ...
        if constants.srcGuildMapping and constants.srcGuildMapping.GroupSaveDataMap:
            for gid, gdata in constants.srcGuildMapping.GroupSaveDataMap.items():
                ...
                for p in players:
                    ...
                    if uid:
                        valid_player_uids.add(
                            str(uid).replace("-", "").lower()
                        )
        ...
        if os.path.exists(players_dir):
            ...
            if player_files:
                def load_player_file(filename):
                    try:
                        p_gvas = sav_to_gvasfile(
                            os.path.join(players_dir, filename)
                        )
                        p_prop = p_gvas.properties.get("SaveData", {}).get(
                            "value", {}
                        )
        ...
        ownership = ContainerOwnership.build(
            cmap, data.get("CharacterContainerSaveData", {}).get("value", [])
        )
        ...
        for item in cmap:
            ...
            if is_worker:
                ...
                if effective and (
                    effective in valid_player_uids
                    or effective in player_containers
                ):
                    u_str = effective
                    is_worker = False
            base = str(
            ...
            e_list = (
                raw.get("EquipWaza", {}).get("value", {}).get("values", [])
            )
            for w in e_list:
                w_short = w.split("::")[-1]
            ...
            active = [
                SKILLMAP.get(w.split("::")[-1].lower(), w.split("::")[-1])
                for w in e_list
            ]
            m_list = (
                raw.get("MasteredWaza", {}).get("value", {}).get("values", [])
            )
            for w in m_list:
                w_short = w.split("::")[-1]
                if w_short.lower() not in SKILLMAP:
            ...
            active_count = (
                sum((1 for s in e_list if s and s.strip()))
                if isinstance(e_list, list)
                else 0
            )
            skills_str = (
                f"Active: {active_count}/3, Passive: {passive_count}/4"
            )
            ...
            if active:
                ...
                info += "    Active Skills:   None\n"
            if pskills:
                info += f"    Passive Skills: {','.join(pskills)}\n"
            else:
                info += "    Passive Skills: None\n"
            if learned:
                info += f"    Learned Skills:  {','.join(learned)}\n"
            else:
                info += "    Learned Skills:  None\n"
            info += f"  IVs:      {iv_str}\n"
            ...
            if is_illegal:
                ...
                learned_skills_list = (
                    list(m_list) if isinstance(m_list, list) else []
                )
                illegal_info = {
                    "name": name,
                    "nickname": nick,
                    ...
                    "passive_skills": list(p_list) if isinstance(p_list, list) else [],
                    "active_skills": list(e_list) if isinstance(e_list, list) else [],
            ...
            if is_worker:
                player_pals_count["worker_dropped"] = (
                    player_pals_count.get("worker_dropped", 0) + 1
                )
            else:
                player_pals_count[u_str] = (
                    player_pals_count.get(u_str, 0) + 1
                )

        if any(miss.values()):
            with open(
                os.path.join(log_folder, "missing_assets.log"),
                "w",
                encoding="utf-8",
            ) as f:
        ...
        for uid, containers in owner_pals_grouped.items():
            ...
            if uid.startswith("WORKER_"):
                ...
                g_name = sanitize_filename(
                    guild_name_map.get(g_id, "Unknown Guild").encode(
                        "utf-8", "replace"
                    ).decode("utf-8")
                )
                g_dir = os.path.join(
                    log_folder, "Guilds", f"({g_id})_({g_name})_({b_count})"
                )
                ...
                lf = os.path.join(
                    p_dir, f"({uid})_({sname})_({pal_count}).log"
                )
            ...
            if not lg.hasHandlers():
                try:
                    h = logging.FileHandler(
                        lf, mode="w", encoding="utf-8", errors="replace"
                    )
            ...
            sorted_keys = prio + sorted(
                [k for k in containers.keys() if k not in prio]
            )
    ...
    def _setup_fresh_logs_folder(self, base_path):
        ...
        logger_dirs = [
            "Scan Save Logger",
            "Illegal Pal Logger/Guilds",
            "Illegal Pal Logger/Players",
        ]
        for logger_dir in logger_dirs:
            full_path = os.path.join(logs_dir, logger_dir)
            os.makedirs(full_path, exist_ok=True)

    def _process_scan_log(
        self, data_source, playerdir, log_folder, guild_name_map, base_path,
        illegal_pals_by_owner=None, owner_nicknames=None,
    ):
        """Generate the scan log text files."""
        # Count owned pals
        owned_counts = {}
        try:
            level_data = constants.loaded_level_json["properties"]["worldSaveData"]["value"]
            char_map = level_data["CharacterSaveParameterMap"]["value"]
            ownership = ContainerOwnership.build(
                char_map, level_data.get("CharacterContainerSaveData", {}).get("value", [])
            )
            for item in char_map:
                try:
                    sp = item["value"]["RawData"]["value"]["object"]["SaveParameter"]["value"]
                    owner_uid = sp.get("OwnerPlayerUId", {}).get("value")
                    effective = ownership.get_effective_owner(
                        item.get("key", {}).get("InstanceId", {}).get("value"), owner_uid
                    )
                    if effective:
                        owned_counts[effective] = owned_counts.get(effective, 0) + 1
                except Exception:
                    continue
        except Exception:
            pass
        ...
        logger_ls = logging.getLogger("LoadSaveLogger")
        logger_ls.handlers.clear()
        logger_ls.setLevel(logging.DEBUG)
        logger_ls.propagate = False
        formatter = logging.Formatter("%(message)s")
        fh = logging.FileHandler(scan_log_path, encoding="utf-8")
        fh.setFormatter(formatter)
        logger_ls.addHandler(fh)
        ...
        players_logger.info("")
        header = (
            f"{'Player Name':<30} | {'Last Seen':<15} | {'Level':<5} | "
            f"{'Pals':<5} | {'UID':<36} | {'Guild ID':<36} | {'Guild Name':<30}"
        )
        players_logger.info(header)
        players_logger.info("-" * 150)
        ...
        if constants.srcGuildMapping:
            for gid, gdata in constants.srcGuildMapping.GroupSaveDataMap.items():
                ...
                guild_leader = (
                    players[0].get("player_info", {}).get("player_name", "Unknown")
                )
                logger_ls.info("=" * 60)
                logger_ls.info(
                    f"Guild: {guild_name} | Guild Leader: {guild_leader} | Guild ID: {gid}"
                )
                logger_ls.info(f"Base Locations: {len(base_ids)}")
                for i, base_id in enumerate(base_ids, 1):
                    basecamp = constants.srcGuildMapping.BaseCampMapping.get(
                        toUUID(base_id)
                    )
                    if basecamp:
                        translation = basecamp["value"]["RawData"]["value"][
                            "transform"
                        ]["translation"]
                        tx, ty, tz = (
                            translation["x"],
                        ...
                        logger_ls.info(
                            f"Base {i}: Base ID: {base_id} | "
                            f"Old: {int(old_c[0])},{int(old_c[1])} | "
                            f"New: {int(new_c[0])},{int(new_c[1])} | "
                            f"RawData: {tx},{ty},{tz}"
                        )
                ...
                with ThreadPoolExecutor() as executor:
                    results = list(
                        executor.map(
                            lambda p: self._top_process_player(p, playerdir, log_folder),
                            players,
                        )
                    )
                for uid, pname, uniques, caught, encounters in results:
                    level = constants.player_levels.get(
                        str(uid).replace("-", ""), "?"
                    )
                    owned = owned_counts.get(
                        str(uid).replace("-", "").lower(), 0
                    )
                    last = next(
                        (
                    ...
                    lastseen = (
                        "Unknown"
                        if last is None
                        else format_duration_short(
                            (tick - int(last)) / 10000000.0
                        )
                    )
                    logger_ls.info(
                        f"Player: {pname} | UID: {uid} | Level: {level} | "
                        f"Caught: {caught} | Owned: {owned} | "
                        f"Encounters: {encounters} | Uniques: {uniques} | "
                        f"Last Online: {lastseen}"
                    )
                    sanitized_pname = self._sanitize_for_alignment(pname)
                    players_logger.info(
                        f"{sanitized_pname:<30} | {lastseen:<15} | {level:<5} | "
                        f"{owned:<5} | {str(uid):<36} | {str(gid):<36} | "
                        f"{guild_name:<30}"
                    )
                    total_players += 1
                    total_caught += caught
        ...
        logger_ls.info("********** PST_STATS_BEGIN **********")
        logger_ls.info(f"Total Players: {total_players}")
        logger_ls.info(f"Total Caught Pals: {total_caught}")
        logger_ls.info(
            f"Total Overall Pals: {total_owned + total_worker_dropped}"
        )
        logger_ls.info(f"Total Owned Pals: {total_owned}")
        logger_ls.info(f"Total Worker/Dropped Pals: {total_worker_dropped}")
        logger_ls.info(f"Total Active Guilds: {active_guilds}")
        logger_ls.info(f"Total Bases: {total_bases}")
        logger_ls.info("********** PST_STATS_END ************")

        for h in logger_ls.handlers[:]:
            logger_ls.removeHandler(h)
            h.close()
        ...
        if self.dps_tasks:
            ...
            with ProcessPoolExecutor(
                ...
                futures = {
                    executor.submit(_process_dps_scan_worker, task): task
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        if pal_info_strings:
                            ...
                            dps_log_path = os.path.join(
                                dps_players_folder, filename
                            )
                            with open(dps_log_path, "w", encoding="utf-8") as dps_log:
                                dps_log.write(
                                    f"{pname}'s {pal_count} Pals\n"
                                )
                                dps_log.write("=" * 40 + "\n")
                                dps_log.write(
        ...
        if illegal_pals_by_owner:
            ...
            for uid_str, location_groups in illegal_pals_by_owner.items():
                ...
                if uid_str.startswith("WORKER_"):
                    ...
                    if len(parts) >= 3:
                        guild_id = parts[1]
                        guild_illegals[guild_id].append(
                            (uid_str, all_pals, location_groups)
                        )
                else:
                    player_illegals[uid_str].append(
                        (uid_str, all_pals, location_groups)
                    )
            ...
            if guild_illegals:
                ...
                for guild_id, base_illegals_list in guild_illegals.items():
                    guild_name = guild_name_map.get(
                        guild_id.lower(), "Unknown Guild"
                    )
                    ...
                    total_illegals = sum(
                        (len(all_pals) for _, all_pals, _ in base_illegals_list)
                    )
                    guild_dir = os.path.join(
                        guilds_illegal_dir,
                        f"({guild_id})_({guild_sname})_({base_count})",
                    )
                    ...
                    for uid_str, all_pals, location_groups in base_illegals_list:
                        ...
                        pname = (
                            owner_nicknames.get(uid_str, f"Base_{base_id}")
                            if owner_nicknames
                            else f"Base_{base_id}"
                        )
                        sname = sanitize_filename(
                            pname.encode("utf-8", "replace").decode("utf-8")
                        )
                        ...
                        log_file = os.path.join(
                            guild_dir,
                            f"({base_id})_({pal_count}_illegals).log",
                        )
                        lname = "".join(
                            c if c.isalnum() or c in ("_", "-") else "_"
                            for c in f"lg_illegal_{uid_str}"
                        )
                        logger_il = logging.getLogger(lname)
                        logger_il.setLevel(logging.INFO)
                        logger_il.propagate = False
                        if not logger_il.hasHandlers():
                            try:
                                h = logging.FileHandler(
                                    log_file,
                                    mode="w",
                                ...
                                h.setFormatter(logging.Formatter("%(message)s"))
                                logger_il.addHandler(h)
                            except Exception:
                                continue
                        self._write_illegal_pal_log(
                            logger_il, pname, pal_count, location_groups,
                            PASSMAP, SKILLMAP,
                        )
                        for h in logger_il.handlers[:]:
                            h.flush()
                            h.close()
                            logger_il.removeHandler(h)
                ...
                for uid_str, illegals_list in player_illegals.items():
                    for _, all_pals, location_groups in illegals_list:
                        ...
                        if owner_nicknames is None:
                            owner_nicknames = {}
                        pname = owner_nicknames.get(
                            uid_str, f"Player_{uid_str[:8]}"
                        )
                        sname = sanitize_filename(
                            pname.encode("utf-8", "replace").decode("utf-8")
                        )
                        ...
                        log_file = os.path.join(
                            players_illegal_dir,
                            f"({uid_str})_({sname})_({pal_count}_illegals).log",
                        )
                        lname = "".join(
                            c if c.isalnum() or c in ("_", "-") else "_"
                            for c in f"lg_illegal_{uid_str}"
    ...
    def _write_illegal_pal_log(self, logger_il, pname, pal_count,
                                location_groups, PASSMAP, SKILLMAP):
        """Write illegal pal entries to the given logger."""
        logger_il.info("=" * 80)
        logger_il.info(f"ILLEGAL PALS LOG: {pname}")
        logger_il.info(f"Total Illegal Pals Found: {pal_count}")
        logger_il.info("=" * 80)
        logger_il.info("")
        prio = ["DPS Storage", "Current Party", "PalBox Storage", "Base Worker"]
        sorted_locations = prio + sorted(
            [k for k in location_groups.keys() if k not in prio]
        )
        for location in sorted_locations:
            ...
            for info in sorted(pals_in_location, key=lambda x: x["name"]):
                ...
                iv_str = (
                    f"HP: {info['talent_hp']}(+0%),ATK: {info['talent_shot']}"
                    f"(+0%),DEF: {info['talent_defense']}(+0%)"
                )
                soul_str = (
                    f"HP Soul: {info['rank_hp']}, ATK Soul: {info['rank_attack']}, "
                    f"DEF Soul: {info['rank_defense']}, Craft: {info['rank_craftspeed']}"
                )
                rank_str = (
                    f"{info.get('rank', 1)} stars ({info.get('rank', 1) - 1}☆)"
                )
                skills_str = (
                    f"Active: {info.get('active_count', 0)}/3, "
                    f"Passive: {info.get('passive_count', 0)}/4"
                )
                ...
                if active_skills_display:
                    info_block += (
                        f"    Active Skills:   {', '.join(active_skills_display)}\n"
                    )
                if passive_skills_display:
                    info_block += (
                        f"    Passive Skills: {', '.join(passive_skills_display)}\n"
                    )
                if learned_skills_display:
                    info_block += (
                        f"    Learned Skills:  {', '.join(learned_skills_display)}\n"
                    )
                else:
                    info_block += "    Learned Skills:  None\n"
                ...
                logger_il.info(info_block)
                logger_il.info("-" * 20)

    def _top_process_player(self, p, playerdir, log_folder):
        """Process a single player entry from the guild data."""
        uid = p.get("player_uid")
        ...
        if os.path.isfile(sav_file):
            try:
                ...
                pal_capture_count_list = record_data.get(
                    "PalCaptureCount", {}
                ).get("value", [])
                uniques = (
                    len(pal_capture_count_list) if pal_capture_count_list else 0
                )
                caught = (
                    sum((e.get("value", 0) for e in pal_capture_count_list))
                    if pal_capture_count_list
                    else 0
                )
                pal_deck_unlock_flag_list = record_data.get(
                    "PaldeckUnlockFlag", {}
                ).get("value", [])
                encounters = (
                    max(len(pal_deck_unlock_flag_list) if pal_deck_unlock_flag_list else 0, uniques)
                )
            except Exception:
                pass
        if os.path.isfile(dps_file):
            ...
            if constants.srcGuildMapping and constants.srcGuildMapping.GroupSaveDataMap:
                for gdata in constants.srcGuildMapping.GroupSaveDataMap.values():
                    ...
                    for p in players:
                        if (
                            str(p.get("player_uid", "")).replace("-", "").lower()
                            == str(uid).replace("-", "").lower()
                        ):
                            player_valid = True
                            break
    ...
    def get_current_stats(self):
        """Return a dict of current save statistics."""
        if not constants.loaded_level_json:
        ...
        total_players = sum(
            (
                ...
                for g in group_data
                if g["value"]["GroupType"]["value"]["value"]
                == "EPalGroupType::Guild"
            )
        )
        total_guilds = sum(
        ...
        for c in char_data:
            ...
            if struct_type == "PalIndividualCharacterSaveParameter":
                if (
                    ...
                    .get("SaveParameter", {})
                    .get("value", {})
                    and val["object"]["SaveParameter"]["value"]["IsPlayer"].get(
                        "value"
                    )
                ):
                    continue
    ...
    def get_players(self):
        """Return a list of (uid, name, guild_id, last_seen, level, elapsed) tuples."""
        if not constants.loaded_level_json:
            return []
        out = []
        wsd = constants.loaded_level_json["properties"]["worldSaveData"]["value"]
        ...
        for g in wsd["GroupSaveDataMap"]["value"]:
            ...
            for p in players:
                ...
                lastseen = (
                    "Unknown"
                    if last is None
                    else format_duration_short(elapsed)
                )
                level = (
                    constants.player_levels.get(uid.replace("-", ""), "?")
                    if uid
                    else "?"
                )
    ...
    def get_guild_name_by_id(self, target_gid):
        ...
        for g in (
            ...
            if (
                g["value"]["GroupType"]["value"]["value"] == "EPalGroupType::Guild"
                and current_gid == target_gid
            ):
    ...
    def is_player_guild_leader(self, guild_id, player_uid):
        ...
        for g in (
            ...
            if (
                g["value"]["GroupType"]["value"]["value"] == "EPalGroupType::Guild"
                and current_gid == guild_id
            ):
                admin_uid = as_uuid(
                    g["value"]["RawData"]["value"].get("admin_player_uid", "")
                )
                return admin_uid == player_uid
        return False

    def _create_player_summary_json(self, data_source, log_folder, guild_name_map):
        """Create a JSON summary of all players."""
        from ..utils import as_uuid
        json_logger_folder = os.path.join(
            os.path.dirname(log_folder), "Json Logger"
        )
        ...
        try:
            ...
            for item in char_map:
                try:
                    ...
                    if effective:
                        owned_counts[effective] = (
                            owned_counts.get(effective, 0) + 1
                        )
                except Exception:
                    continue
        except Exception:
        ...
        if constants.srcGuildMapping:
            for gid, gdata in constants.srcGuildMapping.GroupSaveDataMap.items():
                ...
                for p in players:
                    uid_raw = p.get("player_uid")
                    uid = str(uid_raw) if uid_raw is not None else ""
                    player_name = p.get("player_info", {}).get(
                        "player_name", "Unknown"
                    )
                    last = p.get("player_info", {}).get("last_online_real_time")
                    last_seen = (
...
def _process_dps_scan_worker(args):
    """Worker function for DPS file scanning (runs in process pool)."""
    ...
    try:
        ...
        for entry in save_param_array:
            try:
                ...
                dn = (
                    f"{name}(Nickname: {nick})"
                    if nick != "Unknown" and nick
                    else name
                )
                passive_count = len(p_list) if isinstance(p_list, list) else 0
                active_count = (
                ...
                if is_illegal:
                    illegal_info = {
                        ...
                        "active_count": active_count,
                        "passive_skills": list(p_list) if isinstance(p_list, list) else [],
                        "active_skills": [s for s in e_list if s and s.strip()]
                        if isinstance(e_list, list)
                        else [],
                        "learned_skills": list(m_list) if isinstance(m_list, list) else [],
                        "illegal_markers": illegal_markers,
                        "instance_id": inst_id,
